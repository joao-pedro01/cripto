...
export default function AddCripto({navigation}){
    ...
    const [novaCripto, setNovaCripto] = useState({
        emoji: '💵', nome: '',
        simbolo: '', quantidade: 0, valor: 0, vendido: false,
        valorVenda: 0
    })
    const handleTeclado = (emojiObject) => {
        setNovaCripto({...novaCripto,
            emoji: emojiObject.emoji
        })
    }
    const validaCripto = async() => {
        // Efetuando as validações dos formularios
        if(novaCripto.nome === '') {
            Alert.alert('⚠Atenção', 'O campo nome da criptomoeda é obrigatório')
            return
        }
        if(novaCripto.simbolo.length !== 3) {
            Alert.alert('⚠Atenção', 'O símbolo deve ter 3 caracteres')
            return
        }
        if(parseFloat(novaCripto.valor) <= 0) {
            Alert.alert('⚠Atenção', 'O valor da compra deve ser um número positivo')
            return
        }
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Adicionar Cripto</Text>
            ...
            <EmojiPicker
                ...
                enableSearchBar
                />
            <Text onPress={() => setIsOpen(true)}>Abrir Teclado Emoji</Text>
            <TextInput 
                style={styles.input}
                ...
                onChangeText={(text)=> setNovaCripto({...novaCripto, simbolo: text})}
            />
            <TextInput 
                style={styles.input}
                placeholder='Quantidade Adquirida'
                keyboardType='numeric'
                onChangeText={(text)=> setNovaCripto({...novaCripto, nome: text})}
            />
